...
class TurtleControllerNode(Node): 
    def __init__(self):
        super().__init__("turtle_controller_py") 
        ...
        # self.add_on_set_parameters_callback(self._parameters_callbacks)
        ...
        self.catch_turtles_client_ = self.create_client(CatchTurtle, 'catch_turtle')
    ...
    def call_catch_turtle_service(self, turtle_name):
        while not self.catch_turtles_client_.wait_for_service(1.0):
            self.get_logger().warn("Waiting for kill service...")
            
        request = CatchTurtle.Request()
        request.name = turtle_name
        
        future = self.catch_turtles_client_.call_async(request)
